...
Usage :
    python linkedin_bot.py                                       # envoi actif
    python linkedin_bot.py --dry-run                             # scrape sans envoyer
    python linkedin_bot.py --simulate                            # pipeline complet sans envoyer
    python linkedin_bot.py --test-message <url_profil_linkedin>  # test sur un profil précis

Config :
    Remplir config.json avant de lancer.

Table des matières
──────────────────
  1. Imports & constantes       Chemins de fichiers, logging
  2. Helpers                    load_config, load_seen, save_seen, human_delay
  3. LinkedInBot                Classe principale
     3.1  start / stop          Lancement et fermeture du navigateur WebKit
     3.2  login                 Connexion LinkedIn (auto + fallback manuel)
     3.3  get_followers         Scraping de la liste des abonnés via la modale
     3.4  _navigate_to_compose  Navigation profil → page de composition (partagé)
     3.5  send_message          Envoi d'un message réel
     3.6  simulate_message      Simulation sans envoi
  4. Pipeline                   save_perf, save_all_followers, run()
"""
...
class LinkedInBot:
    ...
    def login(self):
        ...
        log.warning("   2. Valide la vérification sur ton téléphone si demandée")
        ...
        while not self._is_logged_in():
            ...
            if not self._is_logged_in():
                log.warning(f"  Pas encore sur le feed (URL : {self.page.url})")
        log.info("✅ Connecté.")
    ...
    def get_followers(self) -> list[dict]:
        ...
        for text in EXPAND_BTNS:
            try:
                ...
                pass
        if not btn_clicked:
            log.warning("  ⚠️  Bouton 'Afficher tous les abonnés' non trouvé.")
    ...
    def send_message(self, profile_url: str, name: str) -> bool:
        try:
            ...
            if not self.message_template.strip():
                log.error("  ❌ message_template vide dans config.json.")
                return False
...
def save_all_followers(followers: list[dict]):
    with open("all_followers.json", "w", encoding="utf-8") as f:
        json.dump(followers, f, indent=2, ensure_ascii=False)
    log.info(f"📄 {len(followers)} abonné(s) sauvegardé(s) → all_followers.json")
...
def _parse_args():
    ...
    if "--test-message" in sys.argv:
        ...
        if idx + 1 < len(sys.argv):
            ...
            raise SystemExit("Usage : python linkedin_bot.py --test-message <url>")
...
def run():
    ...
    if test_url:       mode_label = f"TEST MESSAGE → {test_url}"
    elif simulate:     mode_label = "SIMULATION (pipeline complet, aucun message envoyé)"
    elif dry_run:      mode_label = "DRY-RUN (scraping seul)"
    else:              mode_label = "ENVOI ACTIF"

    log.info("=" * 50)
    log.info(f"Démarrage pipeline [{mode_label}] — {now_str}")

    perf = {"date": now_str, "mode": mode_label, "login_s": None, "scraping_s": None, "messages": [], "total_s": None}
    ...
    with sync_playwright() as p:
        ...
        try:
            ...
            if test_url:
                ...
                log.info("✅ Message test envoyé." if success else "❌ Échec du message test.")
